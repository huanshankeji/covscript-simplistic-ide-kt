...
fun <A, B> bindBidirectionally(
    ...
    propertyA.bindByOnChange {
        propertyB.value = converter.aToB(it)
    }
    propertyB.bindByOnChange {
        propertyA.value = converter.bToA(it)
    }
}
...
fun <T, P : Property<T>> Property<T?>.bindingWithNullForDefault(
    propertyCreator: () -> P,
    default: T
): P =
    bidirectionalBinding(
        ...
        object : Converter<T?, T> {
            ...
            override fun bToA(b: T): T? =
                if (b == default) null else b
        })

fun <T> SimpleObjectProperty<T?>.bindingWithNullForDefault(
    default: T
): SimpleObjectProperty<T> =
    bindingWithNullForDefault(
        ::SimpleObjectProperty,
        default
    )

fun SimpleObjectProperty<String?>.bindingWithNullForDefault(
    default: String
