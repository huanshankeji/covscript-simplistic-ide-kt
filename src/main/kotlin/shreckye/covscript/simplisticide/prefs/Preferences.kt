...
fun <Data> Preferences.getObject(
    key: String,
    deserializer: StringDeserializer<Data>,
    default: Data? = null
) =
    get(key, null)?.let(deserializer::stringToData)
        ?: default

fun <Data> Preferences.putObject(
    key: String,
    data: Data,
    serializer: StringSerializer<Data>
) =
    put(key, serializer.dataToString(data))

fun <Data> Preferences.putOrRemoveObject(
    key: String,
    data: Data?,
    serializer: StringSerializer<Data>
) =
    if (data !== null) putObject(
        key,
        data,
        serializer
    ) else remove(key)
...
fun Preferences.putOrRemoveDouble(
    key: String,
    value: Double?
) =
    if (value !== null) putDouble(key, value) else remove(
        key
    )