...
private fun joinToCommandLine(vararg commands: String) =
    commands.asSequence()
        .map(::escapeCommandAndQuoteIfNeeded)
        .joinToString(" ")
...
interface TerminalActions {
    ...
    fun runProcessWithTerminal(
        vararg commands: String,
        directory: File
    ): Process =
        startProcess(
            *getRunProcessWithTerminalCommands(*commands),
            directory = directory
        )
    ...
    fun runProcessAndPauseWithTerminal(vararg commands: String): Process =
        startProcess(
            *getRunProcessAndPauseWithTerminalCommands(
                *commands
            )
        )

    fun runProcessAndPauseWithTerminal(
        vararg commands: String,
        directory: File
    ): Process =
        startProcess(
...
object LinuxXTerminalEmulatorActions : TerminalActions {
    override fun getRunNoArgProcessWithTerminalCommands(
        command: String
    ): Array<String> =
    ...
    override fun getRunProcessWithTerminalCommands(vararg commands: String): Array<String> =
        arrayOf(
            "x-terminal-emulator",
            "-e",
            joinToCommandLine(*commands)
        )

    override fun getRunProcessAndPauseWithTerminalCommands(
        vararg commands: String
    ): Array<String> =
...
object MacOSXOpenTerminalActions : TerminalActions {
    ...
    private fun getOpenTerminalInDirectoryCommandsWithPossiblyMoreCommands(
        ...
        arrayOf(
            ...
            joinToCommandLine(
                ...
                """tell app "Terminal" to do script ${
                    escapeAndQuoteAppleScript(
                        joinToCommandLine("cd", "`pwd`",
                            *commands?.let {
                                arrayOf(
                                    ";",
                                    *it
